...
// import LiveTicker from "../LiveTicker";
// import BottomNavbar from "./BottomNavbar";
// import { getMyTotalAmount, getUserPositions } from "../../services/apiService";
...
//     data.forEach((item) => {
//       const totalInvestment = item.buy * item.quantity;
//       const totalProfit = (item.sell - item.buy) * item.quantity;

//       totalPriceSum += totalInvestment;

//       if (totalProfit >= 0) profit += totalProfit;
//       else loss += totalProfit;
//     });
...
//             ...item,
//             totalInvestment: item.buy * item.quantity,
//             totalProfit: (item.sell - item.buy) * item.quantity,
//           }));

//           setPositionsData(processed);
...
//               ) : (
//                 <>
//                   <p className="mb-1 fw-semibold text-secondary">Total Investment</p>
//                   <h5 className="fw-bold">₹{formatAmount(totals.totalPrice)}</h5>

//                   <p className="mb-1 fw-semibold mt-3 text-secondary">Net Portfolio P&L</p>
//                   <h5 className="fw-bold" style={{ color: totals.net >= 0 ? "green" : "red" }}>
//                     ₹{formatAmount(totals.net)}
//                   </h5>

//                   <p className="mb-1 fw-semibold mt-3 text-secondary">Funds Available</p>
//                   <h5 className="fw-bold text-primary">₹{formatAmount(totalAmount)}</h5>
//                 </>
//               )}
//             </Col>
//           </Row>
...
//             variant="primary"
//             size="lg"
//             className="rounded px-4 py-2 fw-semibold w-100"
//             onClick={() => navigate("/userAccount")}
//           >
//             ADD / WITHDRAW FUNDS
//           </Button>
...
const Dashboard = () => {
  ...
  const calculateTotals = (data) => {
    ...
    data.forEach((item) => {
      ...
      const gain = item.gain || 0; // backend se gain field

      totalInvestment += investment;
      netPL += gain; // positive add, negative subtract
    });

    setTotals({
      profit: netPL >= 0 ? netPL : 0,
      loss: netPL < 0 ? netPL : 0,
      net: netPL,
  ...
  return (
    <>
      <Container fluid className="py-3" style={{ backgroundColor: "#f7f8fc", minHeight: "100vh", paddingBottom: "90px" }}>
        <h4 className="fw-bold mb-4" style={{ textAlign: "start" }}>Hi {user.name} 👋</h4>
                ...
                <Card.Body className="p-0">
                  ...
                  {loadingPositions ? (
                    ...
                    <>
                      ...
                      <div className="d-flex justify-content-between">
                        <span className="text-muted small">Net Loss</span>
                        <span className="fw-bold text-danger">₹{formatAmount(totals.loss)}</span>
                      </div>
                    </>
                  )}
                </Card.Body>
